...
### What are Precompiles on Biyaliquid?

On Biyaliquid, precompiles are special, highly-optimized smart contracts embedded directly into our EVM (Ethereum Virtual Machine) layer at the protocol level. Unlike standard Solidity smart contracts that are deployed by users, precompiles are part of the chain's core logic. They are written in Go instead of Solidity and are exposed to the EVM with fixed addresses, making them callable from your Solidity smart contracts just like any other smart contract.

Think of them as native functions of the Biyaliquid chain that have been given an Ethereum-style interface.

### Why are they necessary? (Bridging EVM & Native Modules)

The Biyaliquid EVM doesn't operate in a silo. It's deeply integrated with Biyaliquid's powerful native Cosmos SDK modules, such as the Bank module (for token management), the Exchange module (for the on-chain order book), the Staking module, and more.

Precompiles serve as the crucial **bridge** between the EVM world (where your Solidity contracts live) and these native Biyaliquid functionalities. Without precompiles, your EVM smart contracts would be isolated, unable to tap into the rich features and liquidity of the broader Biyaliquid ecosystem.
...
* **Access to Native Features:** Directly interact with Biyaliquid's unique modules like the on-chain order book, native staking, governance, and the bank module for MTS.
...
* **Seamless Interoperability:** Build truly integrated applications that leverage the strengths of both the EVM and Biyaliquid's Cosmos-native capabilities.
...
By leveraging Foundry's `cast` tool, you can easily deploy contracts and interact with the Biyaliquid chain directly from your terminal. This enables builders to quickly experiment, test, and deploy powerful applications that tap into Biyaliquid's native modules.
...
- How to use Foundry scripts and `cast` commands to deploy and interact with these contracts on Biyaliquid EVM.
- Best practices for bridging EVM logic with Biyaliquid's native features.

Jumpstart your development by cloning the [Biyaliquid Solidity Contracts Repository](https://github.com/biya-coin/solidity-contracts/tree/master/demos) and following the step-by-step guides in each demo directory.
...
When using Foundry, and you "fork" the Biyaliquid Mainnet or Biyaliquid Testnet locally,
...
This occurs becuase Foundry is *simulating* Biyaliquid locally,
rather than actually running on Biyaliquid.
It is therefore running a *generic EVM* simulation,
and not one that is specific to Biyaliquid.
The difference lies in Biyaliquid's native functionality not being present,
and therefore it being unaware of the precompiles.

The fix for this is simple:
Use a version of Foundry that has been patched to include Biyaliquid's precompiles:
