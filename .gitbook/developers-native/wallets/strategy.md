...
- `getAddresses` gets the addresses from the connected wallet strategy. This method returns the Ethereum addresses for Ethereum native wallets (strategies) and Biyaliquid addresses for Cosmos native wallets (strategies).
...
- `signCosmosTransaction` signs an Biyaliquid transaction using the connected wallet strategy,
...
export interface EthereumWalletStrategyArgs {
  chainId: ChainId; // the Biyaliquid chain id
...
// Send an Biyaliquid transaction
export const sendTransaction = async (tx: TxRaw): Promise<string[]> => {
  const response = await walletStrategy.sendTransaction(
    ...
    {address: 'biya1...', chainId: 'biyaliquid-1', sentryEndpoint: 'https://grpc.biyaliquid.network' }
  )
