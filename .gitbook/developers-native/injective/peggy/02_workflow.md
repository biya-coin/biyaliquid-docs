...
1. A fully synced Biyaliquid Chain `Validator` node (`biyaliquidd` process)
...
* Move token assets from Ethereum to Biyaliquid
* Move token assets from Biyaliquid to Ethereum
* Keep the `Peggy.sol` contract in sync with the active `Validator Set` on Biyaliquid
...
* new token batches can be created on Biyaliquid
...
Any asset originating from Ethereum which implements the ERC-20 standard can be transferred from Ethereum to Biyaliquid by calling the `sendToBiyaliquid` function on the [Peggy.sol](https://github.com/biya-coin/peggo/blob/master/solidity/contracts/Peggy.sol) contract which transfers tokens from the sender's balance to the Peggy contract.

The `Operators` all run their `peggo` processes which submit `MsgDepositClaim` messages describing the deposit they have observed. Once more than 66% of all voting power has submitted a claim for this specific deposit representative tokens are minted and issued to the Biyaliquid Chain address that the sender requested.
...
* `Oracle` which observes Ethereum events and sends them as [claims](04_messages.md#Oracle-messages) to Biyaliquid.
* `Relayer` which submits confirmed `Validator Set` updates and `Token Batches` to the `Peggy Contract` on Ethereum
* `Batch Creator` which observes (new) withdrawals on Biyaliquid and decides which of these to batch according to their type and the configured `PEGGO_MIN_BATCH_FEE_USD` value

### Batch Creator

The purpose of the `Batch Creator` is only in creating token batches on the Biyaliquid side. The relevant `Peggy module` RPC is not permissioned so anyone can create a batch.

When a user wants to withdraw assets from Biyaliquid to Ethereum they send a special message to Biyaliquid (`MsgSendToEth`) which adds their withdrawal to `Peggy Tx Pool`.`Batch Creator` continually queries the pool for withdrawals (by token type) and issues a `MsgRequestBatch` to Biyaliquid when a potential batch satisfies the configured `PEGGO_MIN_BATCH_FEE_USD` value (see .env example).
...
Every event emitted by the contract has a unique event nonce. This nonce value is crucial in coordinating `Orchestrators` to properly observe contract activity and make sure Biyaliquid acknowledges them via `Claims`.\
Multiple claims of the same nonce make up an `Attestation` and when the majority (2/3) of orchestrators have observed an event its particular logic gets executed on Biyaliquid.
...
3. `SendToBiyaliquidEvent` - event indicating that a new deposit to Biyaliquid has been initiated
4. `ERC20DeployedEvent` - event indicating a new Cosmos token has been registered on Ethereum

Biyaliquid's `Oracle` implementation ignores the last 12 blocks on Ethereum to ensure block finality. In reality, this means latest events are observed 2-3 minutes after they occurred.
...
In order to deploy the Peggy contract, the validator set of the native chain (Biyaliquid Chain) must be known. Upon deploying the Peggy contract suite (Peggy Implementation, Proxy contract, and ProxyAdmin contracts), the Peggy contract (the Proxy contract) must be initialized with the validator set.\
...
This completes the bootstrap of the Peggy bridge and the chain can be started. Afterward, `Operators` should start their `peggo` processes and eventually observe that the initial `ValsetUpdatedEvent` is attested on Biyaliquid.

### **Updating Biyaliquid Chain validator set on Ethereum**

![img.png](/broken/files/YYRuo8K4QkB7PA2NQuOZ)

A validator set is a series of Ethereum addresses with attached normalized powers used to represent the Biyaliquid validator set (Valset) in the Peggy contract on Ethereum. The Peggy contract stays in sync with the Biyaliquid Chain validator set through the following mechanism:

1. **Creating a new Valset on Biyaliquid:** A new Valset is automatically created on the Biyaliquid Chain when either:
...
2. **Confirming a Valset on Biyaliquid:** Each `Operator` is responsible for confirming Valset updates that are created on Biyaliquid. The `Signer` process sends these confirmations via `MsgConfirmValset` by having the validator's delegated Ethereum key sign over a compressed representation of the Valset data. The `Peggy module` verifies the validity of the signature and persists it to its state.
...
4. **Acknowledging the `ValsetUpdatedEvent` on Biyaliquid:** `Oracle` witnesses the `ValsetUpdatedEvent` on Ethereum, and sends a `MsgValsetUpdatedClaim` which informs the `Peggy module` that the Valset has been updated on Ethereum.
5. **Pruning Valsets on Biyaliquid:** Once a 2/3 majority of validators send their claim for a given `ValsetUpdateEvent`, all the previous valsets are pruned from the `Peggy module` state.
...
### **Transferring ERC-20 tokens from Ethereum to Biyaliquid**

![img.png](/broken/files/IkOQm1tLFSeymMPsomcp)

ERC-20 tokens are transferred from Ethereum to Biyaliquid through the following mechanism:

1. **Depositing ERC-20 tokens on the Peggy Contract:** A user initiates a transfer of ERC-20 tokens from Ethereum to Biyaliquid by calling the `sendToBiyaliquid` function on the Peggy contract which deposits tokens on the Peggy contract and emits a `SendToBiyaliquidEvent`.\
   The deposited tokens will remain locked until withdrawn at some undetermined point in the future. This event contains the amount and type of tokens, as well as a destination address on the Biyaliquid Chain to receive the funds.
2. **Confirming the deposit:** Each `Oracle` witnesses the `SendToBiyaliquidEvent` and sends a `MsgDepositClaim` which contains the deposit information to the Peggy module.
3. **Minting tokens on the Biyaliquid:** Once a majority of validators confirm the deposit claim, the deposit is processed.

* If the asset is Ethereum originated, the tokens are minted and transferred to the intended recipient's address on the Biyaliquid Chain.
* If the asset is Cosmos-SDK originated, the coins are unlocked and transferred to the intended recipient's address on the Biyaliquid Chain.

***

### **Withdrawing tokens from Biyaliquid to Ethereum**

![img.png](/broken/files/6cfRB5jvLu98zFk7AAtH)

1. **Request Withdrawal from Biyaliquid:** A user can initiate the transfer of assets from the Biyaliquid Chain to Ethereum by sending a `MsgSendToEth` transaction to the peggy module.
...
2. **Batch Creation:** A `Batch Creator` observes the pool of pending withdrawals. The batch creator (or any external third party) then requests a batch of to be created for given token by sending `MsgRequestBatch` to the Biyaliquid Chain. The `Peggy module` collects withdrawals matching the token type into a batch and puts it in `Outgoing Batch Pool`.
...
5. **Send Withdrawal Claim to Biyaliquid:** `Oracles` witness the `TransactionBatchExecutedEvent` and send a `MsgWithdrawClaim` containing the withdrawal information to the Peggy module.
