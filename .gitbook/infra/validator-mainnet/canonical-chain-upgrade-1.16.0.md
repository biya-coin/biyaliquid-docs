...
Following [Proposal 541](https://biyahub.com/proposal/541/) This indicates that the upgrade procedure should be performed on block number **127250000**
...
The Biyaliquid Chain will undergo a scheduled enhancement upgrade on **Thursday, July 31st, 2025, 16:00 UTC**.
...
3. Install the [v1.16.0](https://github.com/biya-coin/biyaliquid-chain-releases/releases/tag/v1.16.0-1753404855) binaries.
...
In the event that the upgrade does not succeed, validators and operators must restore the snapshot and downgrade back to Biyaliquid Chain release [v1.15.0-1744722790](https://github.com/biya-coin/biyaliquid-chain-releases/releases/tag/v1.15.0-1744722790) and continue this earlier chain until next upgrade announcement.
...
1.  Verify you are currently running the correct version (`v1.15.0`) of `biyaliquidd`:
...
3. Download and install the `biyaliquid-chain` release for `v1.16.0`:

    ```bash
    wget https://github.com/biya-coin/biyaliquid-chain-releases/releases/download/v1.16.0-1753404855/linux-amd64.zip
    unzip linux-amd64.zip
    sudo mv biyaliquidd peggo /usr/bin
    sudo mv libwasmvm.x86_64.so /usr/lib
    ```

4.  Verify you are currently running the correct version (`v1.16.0`) of `biyaliquidd` after downloading the`v1.16.0` release:
