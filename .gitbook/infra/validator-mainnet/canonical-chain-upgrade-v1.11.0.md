...
Following [proposal 231](https://biyahub.com/proposals/231/) This indicates that the upgrade procedure should be performed on block number **34775000**
...
The Biyaliquid Canonical Chain will undergo a scheduled enhancement upgrade on **Thursday, June 1st, 2023, 14:00 UTC**.
...
3. Install the [v1.11-1685225746](https://github.com/biya-coin/biyaliquid-chain-releases/releases/tag/v1.11-1685225746)
...
In the event that the upgrade does not succeed, validators and operators must restore the snapshot and downgrade back to [Biyaliquid Chain 10010 release](https://github.com/biya-coin/biyaliquid-chain-releases/releases/tag/v1.10.1-1685036881) and continue the chain until next upgrade announcement.

### Upgrade Procedure

## Notes for Validators

Validator operators should configure the **timeout_commit** in **config.toml** to `300ms`. You must remove the wasm cache before upgrading to the new version (rm -rf .biyaliquidd/wasm/wasm/cache/).

1.  Verify you are currently running the correct version (`b578d018c8`) of `biyaliquidd`:
...
3.  Download and install the biyaliquid-chain `10011 release`

    ```bash
    wget https://github.com/biya-coin/biyaliquid-chain-releases/releases/download/v1.11-1685225746/linux-amd64.zip
    unzip linux-amd64.zip
    sudo mv biyaliquidd peggo /usr/bin
    sudo mv libwasmvm.x86_64.so /usr/lib
    ```
4.  Verify you are currently running the correct version (`f77eb5cfd7`) of `biyaliquidd` after downloading the 10009 release:
